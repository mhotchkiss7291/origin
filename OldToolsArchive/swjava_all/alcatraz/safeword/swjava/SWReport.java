...
public class SWReport extends RCRecord {
  ...
  public Vector filterRecords(long time) {
    ...
    try {
      sendAndWait("grep =" + RequestType   
                    + " /usr/safeword/swjava/RecsAll" + now + ".rec" 
                    + " > /usr/safeword/swjava/RecsFiltered" + now + ".rec", 3); 		//CHECK
    ...
    try {
      reader = 
				new BufferedReader(
					new FileReader(
						"/usr/safeword/swjava/RecsFiltered" + now + ".rec"));
		//CHECK
	...
	if(Email.length() >= 3){
          SendEmail se = null;
          se = new SendEmail(Email, replyV, RequestType);
