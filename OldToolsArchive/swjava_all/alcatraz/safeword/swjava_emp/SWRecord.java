...
public class SWRecord {
  ...
  SWRecord( Vector in ) {
    ...
    // If valid RequestType isn't present, send alert!
      ...
      Notify n = new Notify("Illegal request type: " + RequestType );
      try {
        n.send();
      } catch (IOException e ) {
        e.printStackTrace();
      ...
      try {
        ...
        try {
          SWErrorHandler seh = new SWErrorHandler("CLI_ERROR", 
              "Error while loading the CLI on alcatraz EMP" + cmd);
        } catch (IOException e) {
          e.printStackTrace();
        }
      }
  ...
  public Vector verifyAccount() {
    ...
    NotifyPINUser npu = new NotifyPINUser( EMail );
    try {
      npu.send();
    } catch (IOException e) {
      e.printStackTrace();
  ...
  public void log( String in ) {
    ...
    try {
      ...
      e.printStackTrace();
      Notify n = new Notify("Can't open log file");
      try {
        n.send();
      } catch (IOException ee) {
        ee.printStackTrace();
  ...
  public void copyRecordFiles(String Recfile) {
    ...
    try {
      ...
      try {
        ...
        try {
          SWErrorHandler seh = new SWErrorHandler("Copy Error:", 
