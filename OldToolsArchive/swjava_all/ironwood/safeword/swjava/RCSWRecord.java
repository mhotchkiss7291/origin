...
public class RCSWRecord {
  ...
  RCSWRecord( Vector in ) {
    ...
    if ( RequestType.equals("GETSERVICES") ) {
      ...
      Safew = (String)requestV.elementAt(10);
      Ite = (String)requestV.elementAt(11);
  ...
  public Vector connectToServer() {
    ...
    try {
      ...
      try {
        ...
        try {
          RCSWErrorHandler seh = new RCSWErrorHandler("CLI_ERROR", 
              "Error while loading the CLI on ironwood RC" + cmd);
        } catch (IOException e) {
          e.printStackTrace();
        }
        return replyV;
  ...
  private Vector getServicesFromFile() {
    ...
    replyV.setElementAt( RecordData[9], 2 );
		...
		replyV.setElementAt( Safew, 10 );
		replyV.setElementAt( Ite, 11 );
  ...
  // Socket request made to RCListener from a protected
  ...
  private synchronized Vector sendAndWait(String s, int waitTime) 
    ...
    if (outputFromServer == null)
      throw new RCSafewordException("ERROR Timeout in CLI connection.");
    ...
    if (!d.isEmpty() && ((String)d.lastElement()).startsWith(completed)) {
      ...
      if (outputFromServer.indexOf("ERROR") == 0) {
        throw new RCSafewordException(outputFromServer);
      }
    }
  ...
  public void copyRecordFiles(String Recfile) {
    ...
    try {
      ...
      try {
        ...
        try {
          RCSWErrorHandler seh = new RCSWErrorHandler("Copy Error:",
