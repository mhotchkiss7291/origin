...
//main class for a user to activate a replacement token card
public class eSunAuthReplace extends HttpServlet {
    ...
    UserID = safeID.toLowerCase();
		...
		RequestType = new String("REPLACE");
...
/****End of repeat call to ironwood*****/
		...
		if( s.equals("REPLACECONFIRMED") ) {
			...
			String s2 = (String) replyVector.elementAt(0);
	 		...
	 		if(s2.equals("REPLACECONFIRMED")) {
				...
				out.println(
					"Your replacement token card has been successfully registered.");
				out.println("<P>");
				...
				requestVector.setElementAt("REPLACE", 5);
	...
	private Vector callSafeword(String IP, int port, Vector v, PrintWriter out) {
