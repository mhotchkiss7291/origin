...
public class RCARecord {
  ...
  RCARecord( Vector in ) {
    ...
    // If valid RequestType isn't present, send alert!
      ...
      RCANotify n = new RCANotify("Illegal request type: " + RequestType );
      try {
        n.send();
      } catch (IOException e ) {
        e.printStackTrace();
  ...
  public Vector connectToServer() {
    ...
    try {
      ...
      try {
        ...
        try {
          RCAErrorHandler seh = new RCAErrorHandler(
						"CLI_ERROR", "Error while loading the RCA CLI " + cmd);
        } catch (IOException e) {
          e.printStackTrace();
        }
        return replyV;
      ...
      try {
        RCAErrorHandler seh = new RCAErrorHandler("CLI_ERROR", 
  ...
  private void readHash( String[] v ) {
	...
	public void getAllRecords(long now) {
		...
		try {
			sendAndWait(
				"get -all -f /usr/safeword-async/swjava/RecsAll" + 
				now + ".rec", 450);   
			//check
  ...
  public Vector enable() {
    ...
    try {
      ...
      } catch (RCASafewordException sse) {
        sse.printStackTrace();
  ...
  private synchronized Vector sendAndWait(String s, int waitTime) 
    ...
    if (outputFromServer == null)
      throw new RCASafewordException("ERROR Timeout in CLI connection.");
    ...
    if (!d.isEmpty() && ((String)d.lastElement()).startsWith(completed)) {
      ...
      if (outputFromServer.indexOf("ERROR") == 0) {
        throw new RCASafewordException(outputFromServer);
      }
    }
  ...
  public void log( String in ) {
    ...
    try {
      ...
      e.printStackTrace();
      RCANotify n = new RCANotify("Can't open log file");
      try {
        n.send();
      } catch (IOException ee) {
        ee.printStackTrace();
  ...
  public void copyRecordFiles(String Recfile) {
    ...
    try {
      ...
      try {
        ...
        try {
          RCAErrorHandler seh = new RCAErrorHandler("Copy Error:",
